...
@router.post("/rooms")
async def create_room(room_data: RoomCreate,db: AsyncSession = Depends(get_db),  owner: User = Depends(get_owner)):
    room_service = RoomService(db=db)
    new_room = await room_service.create_room(data=room_data, owner=owner)
    return new_room

@router.post("/add_tenants")
async def add_tenants(room_id: str, tenant: RegisterRequest, db: AsyncSession = Depends(get_db), owner: User = Depends(get_owner)):
    room_service = RoomService(db=db)
